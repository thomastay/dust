...
fn get_name_percent(node: &Node, indent: &str, bar: &str, dd: &DisplayData) -> (String, String) {
    if bar != "" {
        let percent_size_str = format!("{:.0}%", dd.percent_size(node) * 100.0);
        let percents = format!("│{} │ {:>4}", bar, percent_size_str);
        let name_and_padding = pad_or_trim_filename(node, indent, dd);
        (percents, name_and_padding)
    } else {
        let n = get_printable_name(&node.name, dd.short_paths);
        let name = maybe_trim_filename(n, dd);
        ("".into(), name)
    }
}
